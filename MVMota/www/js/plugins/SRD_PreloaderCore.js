 ...
 * @param 缓存的音频
 * @param Cache Audio
 * @desc 保存以前的音频对象，以便快速检索Keeps previous audio objects stored for quick retrieval.
 * 列出所有将使用缓存的音频文件夹List all the audio folders that will use caching.
 * @default bgm, bgs, me
 *
 * @param 预加载BGM
 * @param Preload BGM
 * @desc 确定哪些BGM预加载Determines which BGM are preloaded. Choices are:
 ...
 * 这是一个自定义文件夹路径，以引用在/img/文件夹中预加载的图像。
 * @default
 *
 * @param 预加载图像1
 * @param Preload Images 1
 * @desc Determines which images are preloaded from Path 1.确定从路径1预加载的图像
 *  all  |  important  |  custom: f1, f2, ...  |  none
 * @default
 *
 * @param == Custom Path 2 ==
 ...
 * 对于所有的音频和img文件夹，您有四个选项用于预加载文件:没有，自定义，重要，全部。
 *
 *   none
 *
 ...
 * 如果设置为“重要”，那么被认为“重要”的文件将被预先加载。对于每个文件夹来说，
 * “重要的”是不同的。例如，重要的BGMs包括在数据库中指定的，
 * 重要的动画图像包括动画中使用的所有图像，等等。
 *
 *   all
 *
...
Imported["SumRndmDde Preloader Core"] = 1.01;
...
Scene_Preload.prototype.createSubtitle = function() {
	this._subHeight = Graphics.boxHeight / 50;
	this._subtitle = new Sprite(new Bitmap(Graphics.boxWidth, this._subHeight));
	this._subtitle.y = Graphics.boxHeight * (960/1000);
...
Scene_Preload.prototype.updateAudio = function() {
	...
	if(audio) {
		...
		// const value = _.format.replace();
		const value = _.format.replace(/%1/, "/audio/" + type + "/" + audio + this._audioExt);//有修改
...
Scene_Preload.prototype.updateImage = function() {
	...
	if(image) {
		...
		// const value = _.format.replace();
		const value = _.format.replace(/%1/, path + picture + ".png" + hueStr);//（有修改）
		this._subtitle.bitmap.drawText(value, 0, 0, Graphics.boxWidth, this._subHeight, 'center');
	}
	this._curIndex++;
